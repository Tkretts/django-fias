...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--remote-file', action='store_true', dest='remote',
                    default=False,
                    help='Download full archive and load it into DB'),
        make_option('--force-replace', action='store_true', dest='force',
                    default=False,
                    help='Force replace database'),
        make_option('--really-replace', action='store_true', dest='really',
                    default=False,
                    help=('If data exist in any table, you should confirm their '
                          'removal and replacement, as this may result in the '
                          'removal of related data from other tables!')),
        make_option('--update', action='store_true', dest='update',
                    default=False,
                    help='Fetch `ver` and load into db'),
        make_option('--fill-weights', action='store_true', dest='weights',
                    default=False,
    ...
    def handle(self, *args, **options):
        ...
        if (path or remote) and Status.objects.count() > 0 and not really:
            self.error('One of the tables contains data. Truncate all FIAS '
                       'tables manually or enter key --really-replace, to clear'
                       ' the table by means of Django ORM')
